...
export default function ResultDashboard() {
  ...
  const name = results && results.length > 0 ? results[0].studentName : "Unknown";
  const regNo = results && results.length > 0 ? results[0].registerNo : "Unknown";

  const availableSems = results ? results.map(r => r.semester).sort((a, b) => a - b) : [];

  const handleSemClick = (sem) => {
    setOpen(false);
    navigate("/result-view", { state: { results, semester: sem } });
  };
