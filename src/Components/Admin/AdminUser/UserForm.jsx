...
function UserForm({ mode, editingUser, onSave, onClose, setMessage }) {
  ...
  const handleSubmit = () => {
    ...
    if (!form.name) {
      setMessage({ open: true, text: "Name is required", type: "warning" });
      return;
    }

    if (form.role?.toLowerCase() === "student") {
      if (!form.year || !form.dept || !form.sec) {
        setMessage({ open: true, text: "Year, Department and Section are required for Student", type: "warning" });
        return;
      }
    }
          ...
          <>
            ...
            <div style={rowStyle}>
              <div>
                ...
                <input name="email" value={form.email} onChange={handleChange} disabled={mode === "edit"} style={inputStyle} />
              </div>

              <div>
                <label style={labelStyle}>Date of Birth</label>
                <input type="date" name="DOB" value={form.DOB} onChange={handleChange} disabled={mode === "edit"} style={inputStyle} />
              </div>
            </div>
            ...
            <div style={rowStyle}>
              ...
              <div>
                ...
                <input type="number" name="currentSemester" value={form.currentSemester || ""} onChange={handleChange} style={inputStyle} />
              </div>
            </div>
          </>
        );
  ...
  return (
    <div className="fixed inset-0 bg-black/45 flex items-center justify-center z-[1000] ">
      <div className="bg-white w-[540px] p-[20px] rounded-[12px]">
        ...
        <select
          name="role"
          value={form.role?.toLowerCase() || "student"}
          onChange={handleChange}
          disabled={mode === "edit"}
