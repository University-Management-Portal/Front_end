...
function StudentCourses() {
  ...
  const fetchStudentCourses = async () => {
    try {
      ...
      const res = await axiosInstance.get(`/academic/courses/department/${dept}`);

      const formatted = res.data.map(c => ({
        id: c.id,
        slug: c.courseCode.toLowerCase(),
        sub: `${c.courseCode} / ${c.courseName}`,
        staff: c.staffList.length > 0 ? c.staffList[0].name : "Not Assigned",
        img: "Advancednet.jpg"
      }));
